...
export default function Page() {
  ...
  const [webcamPosition, setWebcamPosition] = useState<"custom-right" | "bottom-left" | "top-left" | "top-right">("custom-right");
  const [cursorIcon, setCursorIcon] = useState<"default" | "hand" | "zoom" | "text" | "grab" | "grabbing">("default");
  ...
  const [theme, setTheme] = useState<"galactic" | "x" | "dark" | "light">("x");

  const [recognition, setRecognition] = useState<SpeechRecognition | null>(null);

  // Speech Recognition for Auto-Captions (4 Words Max)
  useEffect(() => {
    if (!("webkitSpeechRecognition" in window) || !isSpeechOn) return;
  ...
  // Galactic Theme Particle Animation
  useEffect(() => {
    if (theme !== "galactic") {
      setParticles([]);
      return;
  ...
  // Canvas Drawing with Auto-Zoom and Captions
  useEffect(() => {
  ...
  // Auto-Zoom Logic for Recorded Screen Only
  const handleMouseMove = (e: MouseEvent) => {
    ...
    let fracY = Math.max(0, Math.min(1, (screenY / window.screen.height))); // Fixed: Removed extra parenthesis
    ...
    let confidence = 0.5;
    let newCursorIcon: typeof cursorIcon = "default";
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "fixed", top: 20, right: 20, display: "flex", gap: 10, zIndex: 10001 }}>
        <button onClick={() => setTheme("galactic")} style={floatingButtonStyle(theme === "galactic" ? "#00cc99" : themeStyles.buttonBg, themeStyles.textColor)} title="Galactic Theme">🌌</button>
        <button onClick={() => setTheme("x")} style={floatingButtonStyle(theme === "x" ? "#1DA1F2" : themeStyles.buttonBg, themeStyles.textColor)} title="X Theme">X</button>
        <button onClick={() => setTheme("dark")} style={floatingButtonStyle(theme === "dark" ? "#fff" : themeStyles.buttonBg, themeStyles.textColor)} title="Dark Theme">🌙</button>
        <button onClick={() => setTheme("light")} style={floatingButtonStyle(theme === "light" ? "#333" : themeStyles.buttonBg, themeStyles.textColor)} title="Light Theme">☀️</button>
      </div>

      <div style={{ position: "fixed", right: 20, top: "50%", transform: "translateY(-50%)", display: "flex", flexDirection: "column", gap: 10, zIndex: 10000 }}>
        <button onClick={() => setZoomMode(zoomMode === "off" ? "auto" : "off")} style={floatingButtonStyle(zoomMode === "off" ? "#888" : themeStyles.selectorBg, themeStyles.textColor)} title={zoomMode === "off" ? "Enable Auto Zoom" : "Disable Zoom"}>
          {zoomMode === "off" ? "🔍" : "🚫"}
        </button>
        <button onClick={() => setZoomMode(zoomMode === "grid" ? "auto" : "grid")} style={floatingButtonStyle(zoomMode === "grid" ? "#ffaa33" : themeStyles.selectorBg, themeStyles.textColor)} title={zoomMode === "grid" ? "Switch to Auto Zoom" : "Switch to Grid Zoom"}>
          {zoomMode === "grid" ? "🌐" : "📊"}
        </button>
        {isRecording && (
          <>
            <button onClick={isPaused ? resumeRecording : pauseRecording} style={floatingButtonStyle(isPaused ? "#00cc99" : "#ff3366", themeStyles.textColor)} title={isPaused ? "Resume" : "Pause"}>
              {isPaused ? (
            ...
            <button onClick={takeSnapshot} style={floatingButtonStyle(themeStyles.accentColor, themeStyles.textColor)} title="Snapshot">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={themeStyles.textColor} strokeWidth="2">
                <path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z" /><circle cx="12" cy="13" r="4" />
              </svg>
            </button>
            <button onClick={() => setIsHighlightOn(!isHighlightOn)} style={floatingButtonStyle(isHighlightOn ? "#ff3366" : themeStyles.buttonBg, themeStyles.textColor)} title="Highlight">
        ...
        <button onClick={() => setIsWebcamOn(!isWebcamOn)} style={floatingButtonStyle(isWebcamOn ? themeStyles.buttonBg : "#888", themeStyles.textColor)} title={isWebcamOn ? "Disable Webcam" : "Enable Webcam"}>
          {isWebcamOn ? "📷" : "🚫📷"}
        </button>
        <button onClick={() => setIsSpeechOn(!isSpeechOn)} style={floatingButtonStyle(isSpeechOn ? themeStyles.buttonBg : "#888", themeStyles.textColor)} title={isSpeechOn ? "Disable Speech" : "Enable Speech"}>
          {isSpeechOn ? "🎤" : "🚫🎤"}
        </button>
        <select
          value={webcamPosition}
          onChange={(e) => setWebcamPosition(e.target.value as "custom-right" | "bottom-left" | "top-left" | "top-right")}
          style={{
            width: 40,
            height: 40,
            borderRadius: "50%",
      ...
      <div style={{ position: "fixed", bottom: 20, right: 20, zIndex: 9999, width: isGridExpanded ? gridWidth + 40 : 40, transition: "width 0.3s ease" }}>
        {isGridExpanded ? (
          <div style={{ background: themeStyles.gridBg, borderRadius: 20, padding: "20px", boxShadow: `0 4px 20px ${themeStyles.accentColor}33`, width: gridWidth }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "10px" }}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => setShowSettings(!showSettings)} style={floatingButtonStyle(themeStyles.settingsBg, themeStyles.textColor)} title="Settings">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={themeStyles.textColor} strokeWidth="2">
                ...
                <div style={{ display: "flex", alignItems: "center", cursor: "pointer", background: themeStyles.primaryColor, borderRadius: "50%", width: 24, height: 24, justifyContent: "center", boxShadow: `0 0 10px ${themeStyles.accentColor}40` }} onClick={() => setIsGridExpanded(false)}>
                  <svg viewBox="0 0 24 24" style={{ width: 20, height: 20, color: themeStyles.textColor }}><g><path d="M12 11.59L3.96 3.54 2.54 4.96 12 14.41l9.46-9.45-1.42-1.42L12 11.59zm0 7l-8.04-8.05-1.42 1.42L12 21.41l9.46-9.45-1.42-1.42L12 18.59z" /></g></svg>
                </div>
              </div>
            </div>
            <div style={{ width: gridWidth, height: gridHeight - 80, position: "relative" }} onMouseMove={handleGridMouseMove} onMouseEnter={() => { setIsGridHovered(true); if (zoomMode === "grid") setZoomScale(1.5); }} onMouseLeave={handleGridMouseLeave}>
          ...
          <div style={{ width: 40, height: 40, borderRadius: "50%", background: themeStyles.collapseBg, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: `0 4px 10px ${themeStyles.accentColor}33` }} onClick={() => setIsGridExpanded(true)}>
            <svg viewBox="0 0 24 24" style={{ width: 20, height: 20, color: themeStyles.primaryColor }}><g><path d="M12 4.41L2.54 13.86l1.42 1.42L12 7.59l8.04 7.69 1.42-1.42L12 4.41zm0 14.18l-8.04-8.05-1.42 1.42L12 21.41l9.46-9.45-1.42-1.42L12 18.59z" /></g></svg>
          </div>
        )}
      </div>

      {showSettings && isGridExpanded && (
        <div style={{ position: "fixed", bottom: gridHeight + 60, right: 20, width: 300, padding: "20px", background: themeStyles.gridBg, borderRadius: 20, boxShadow: `0 0 70px ${themeStyles.accentColor}99`, zIndex: 10000, color: themeStyles.textColor }}>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <input type="range" min="1" max="10" step="0.1" value={zoomScale} onChange={(e) => setZoomScale(parseFloat(e.target.value))} style={{ width: "100%", accentColor: themeStyles.accentColor }} />
          </div>
          <div>
            <label style={{ display: "block", marginBottom: 5, color: themeStyles.textColor }}>Grid Opacity:</label>
            <input type="range" min="0.1" max="1" step="0.1" value={gridOpacity} onChange={(e) => setGridOpacity(parseFloat(e.target.value))} style={{ width: "100%", accentColor: themeStyles.accentColor }} />
          </div>
        </div>
      )}

      <main style={{ maxWidth: 1280, width: "100%", position: "relative", margin: "100px auto 0" }}>
        <div style={{ position: "relative", width: "100%", maxWidth: 1280, height: 720, background: themeStyles.collapseBg, borderRadius: 40, overflow: "hidden", boxShadow: `0 40px 100px ${themeStyles.accentColor}80, inset 0 0 40px ${themeStyles.secondaryColor}`, border: `2px solid ${themeStyles.primaryColor}66` }}>
          <canvas ref={canvasRef} style={{ width: "100%", height: "100%", background: themeStyles.gridBg, display: isRecording ? "block" : "none" }} />
          {isRecording && (
            <div style={{ position: "absolute", top: 30, left: 30, padding: "14px 28px", background: themeStyles.secondaryColor, borderRadius: 14, fontSize: "1.3rem", color: themeStyles.textColor, textShadow: `0 0 12px ${themeStyles.primaryColor}cc` }}>
              Live
            </div>
          )}
        </div>
        ...
        {videoURL && (
          <div style={{ position: "relative", width: "100%", maxWidth: 1280, height: 600, marginTop: 80, background: themeStyles.collapseBg, borderRadius: 40, overflow: "hidden", boxShadow: `0 40px 100px ${themeStyles.accentColor}80, inset 0 0 40px ${themeStyles.secondaryColor}`, border: `2px solid ${themeStyles.primaryColor}66` }}>
            <h2 style={{ textAlign: "center", margin: "30px 0", fontSize: "2.2rem", color: themeStyles.primaryColor, textShadow: `0 0 20px ${themeStyles.primaryColor}cc` }}>Playback</h2>
            <video ref={videoRef} src={videoURL} controls onTimeUpdate={handleTimeUpdate} style={{ width: "100%", height: 540, background: themeStyles.gridBg }} />
            <div style={{ position: "absolute", top: 60, left: 0, width: "100%", height: 540, pointerEvents: "none" }}>{overlayDot && <div style={overlayDotStyle()} />}</div>
            <div style={{ position: "absolute", bottom: 30, right: 30, padding: "14px 28px", background: themeStyles.secondaryColor, borderRadius: 14, fontSize: "1.2rem", color: themeStyles.textColor, textShadow: `0 0 12px ${themeStyles.primaryColor}cc` }}>Recorded</div>
            <div style={{ position: "absolute", bottom: 30, left: 30, display: "flex", gap: 30 }}>
              <button onClick={() => videoRef.current?.play()} style={{ ...buttonStyle(themeStyles.buttonBg, themeStyles.textColor), boxShadow: `0 0 35px ${themeStyles.accentColor}80`, transition: "all 0.3s ease" }} onMouseEnter={(e) => (e.currentTarget.style.boxShadow = `0 0 45px ${themeStyles.accentColor}`)} onMouseLeave={(e) => (e.currentTarget.style.boxShadow = `0 0 35px ${themeStyles.accentColor}80`)}>
                Replay
              </button>
              <a href={downloadURL} download={`recording-${sessionId}.webm`} style={{ ...buttonStyle(themeStyles.accentColor, themeStyles.textColor), boxShadow: `0 0 35px ${themeStyles.accentColor}80`, transition: "all 0.3s ease", textDecoration: "none", lineHeight: "3.2rem" }} onMouseEnter={(e: any) => (e.currentTarget.style.boxShadow = `0 0 45px ${themeStyles.accentColor}`)} onMouseLeave={(e: any) => (e.currentTarget.style.boxShadow = `0 0 35px ${themeStyles.accentColor}80`)}>
                Download
              </a>
            </div>
          </div>
...
function buttonStyle(bgColor: string, textColor: string): React.CSSProperties {
  return {
    padding: "12px 24px",
    borderRadius: 8,
    background: bgColor,
...
function floatingButtonStyle(bgColor: string, textColor: string): React.CSSProperties {
  return {
    width: 40,
    height: 40,
    borderRadius: "50%",
